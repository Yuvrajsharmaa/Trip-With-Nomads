...
async function run() {
    // We already have Supabase tools, but for consistency let's use the fetch script approach or just output a list manually matched
    // Wait, I can't easily import @supabase/supabase-js if it's not working, but wait, it WAS in package.json.
    // The previous error was specifically about 'dotenv'.

    // Let's use node-fetch to call Supabase REST API directly if needed, or just hardcode the data I got from the SQL tool.
    const supabaseTrips = [
    ...
    // 3. Compare
    console.log("Trip Verification Report");
    console.log("========================\n");
    console.log(`${"Trip Name (PDF)".padEnd(45)} | ${"Supabase".padEnd(10)} | ${"Framer".padEnd(10)}`);
    console.log("-".repeat(75));

    LIVE_TRIPS_PDF.forEach(pdfTrip => {
        const inSupabase = supabaseTrips.find(st => {
            const lowPdf = pdfTrip.toLowerCase();
            const lowSt = st.title.toLowerCase();
            return lowPdf.includes(lowSt) || lowSt.split(' ').every(part => lowPdf.includes(part));
        });

        const inFramer = framerItems.find(fi => {
            const title = fi.fieldData.edpZYc3f0.value.toLowerCase();
            const lowPdf = pdfTrip.toLowerCase();
            return lowPdf.includes(title) || title.split(' ').every(part => lowPdf.includes(part));
        });

        console.log(`${pdfTrip.padEnd(45)} | ${inSupabase ? "✅ OK" : "❌ MISSING"} | ${inFramer ? "✅ OK" : "❌ MISSING"}`);
    });
}
