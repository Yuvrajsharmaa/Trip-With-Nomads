...
async function postLeadWithSource(
    ...
    const identity = `${source}|${email}|${params.get("slug") || ""}|${window.location.pathname}`;
    const submittedKey = `${LEAD_SUBMITTED_PREFIX}:${identity}`;
    const abandonedKey = `${LEAD_ABANDON_PREFIX}:${identity}`;
...
function leadIdentityForSource(source: string, email: string): string {
    const params = new URLSearchParams(window.location.search);
...
function discoverLeadFormCandidate(): HTMLFormElement | null {
...
async function postPartialLeadIfNeeded(
    ...
    const root = form ?? discoverLeadFormCandidate() ?? document;
    const email = normalizeEmail(
        findInputValue(root, [
            'input[type="email"]',
            'input[name="email"]',
    ...
    const identity = leadIdentityForSource(source, email);
    const submittedKey = `${LEAD_SUBMITTED_PREFIX}:${identity}`;
    const abandonedKey = `${LEAD_ABANDON_PREFIX}:${identity}`;
    if (sessionStorage.getItem(submittedKey) === "1") return false;
    if (sessionStorage.getItem(abandonedKey) === "1") return false;
            ...
            return (
                <Component
                    ...
                    onClick={async (event: any) => {
                        props.onClick?.(event);
                        const form = resolveFormFromEvent(event);
                        await postLeadWithSource(form, source);
                    }}
                    onSubmit={async (event: any) => {
                        props.onSubmit?.(event);
                        const form = resolveFormFromEvent(event);
...
/**
 * withLeadTracking — Backward-compatible generic lead tracking override.
 * Keeps older Framer wiring working for general lead forms.
 */
export function withLeadTracking(Component: ComponentType): ComponentType {
    const SubmitTracked = withSubmitLeadTracking("general_lead")(Component);
    return withLeadAbandonTracking("general_lead")(SubmitTracked);
}
...
/**
 * withCustomTripLeadTracking — Apply to custom-trip lead capture forms.
 * Sends lead submissions and captures abandoned fills (partial_fill)
 * with source = "custom_trip_lead".
 */
export function withCustomTripLeadTracking(Component: ComponentType): ComponentType {
    const SubmitTracked = withSubmitLeadTracking("custom_trip_lead")(Component);
    return withLeadAbandonTracking("custom_trip_lead")(SubmitTracked);
}

/**
 * Apply these on lead forms (or their outer wrappers) to capture abandoned leads.
 * They log to record-lead with status = "partial_fill" once per source+email+page.
 */
export const withBookingInviteAbandonTracking = withLeadAbandonTracking("booking_invite");
export const withTripPageLeadAbandonTracking = withLeadAbandonTracking("trip_page_lead");
export const withCustomTripLeadAbandonTracking = withLeadAbandonTracking("custom_trip_lead");
