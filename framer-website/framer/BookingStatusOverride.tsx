...
type BookingFetchResult =
  | { data: BookingData; error?: undefined }
...
function payableNowAmount(d: BookingData): number {
  const explicit = Math.max(0, toNumber((d as any)?.payable_now_amount));
  if (explicit > 0) return explicit;
  const total = resolvedTotalAmount(d);
  const mode = String((d as any)?.payment_mode || "").trim().toLowerCase();
  return mode === "partial_25" ? Math.round(total * 0.25 * 100) / 100 : total;
}

function appliedCouponCode(d: BookingData): string {
  const snapshot = (d as any)?.coupon_snapshot;
  if (!snapshot || typeof snapshot !== "object") return "";
  const snapshotSource = String(
    snapshot?.applied_discount_source || snapshot?.final_applied_source || "",
  ).trim().toLowerCase();
  if (snapshot?.applied_coupon !== true && snapshotSource !== "coupon") {
    return "";
  }
  return String(
    snapshot?.applied_discount_code ||
      (snapshotSource === "coupon" ? d.coupon_code : "") ||
      "",
  ).trim().toUpperCase();
}
...
export function withBookingStatus(Component): ComponentType {
  return (props: any) => {
    useEffect(() => {
      ...
      const fetchBooking = (): Promise<BookingFetchResult> => {
        ...
        return fetchGateway("get-booking-status", {
          ...
          .then((statusRes) => {
            ...
            return statusRes.json()
              .then((payload) => {
                if (payload?.booking) {
                  return { data: payload.booking as BookingData };
                }
  ...
  return function (Component: ComponentType): ComponentType {
    return (props: any) => {
      ...
      const text = useMemo(() => {
        ...
        return fallback;
      }, [isHydrated, state, data, _pendingCountdown, _pendingTimedOut]);
...
export function withPaymentAmountLabel(Component): ComponentType {
  return textOverride((d) =>
    d.payment_status === "paid" ? "Paid now" : "Payable now"
  )(Component);
}
...
export function withTotalPaid(Component): ComponentType {
  return textOverride((d) => {
    ...
    return fmt(paid > 0 ? paid : payableNowAmount(d));
  })(Component);
}

export function withPayableNowAmount(Component): ComponentType {
  return textOverride((d) => fmt(payableNowAmount(d)))(Component);
}
...
export function withDueLabel(Component): ComponentType {
  return textOverride((d) =>
    String((d as any)?.payment_mode || "").trim().toLowerCase() === "partial_25"
      ? "Remaining due"
      : "Balance due"
  )(Component);
}
...
export function withHeadingText(Component): ComponentType {
  return textOverride(
    (d) => {
      ...
      if (d.payment_status === "failed") return "Payment Failed";
      if (_pendingTimedOut) return "Payment not completed";
      return "Payment processing\u2026";
    },
    "Loading\u2026",
  )(Component);
...
export function withSubheadingText(Component): ComponentType {
  return textOverride(
    (d) => {
      ...
      if (_pendingTimedOut) {
        return "We couldn't confirm a completed payment. Your booking is still safe; you can try again below.";
      }
      if (_pendingCountdown > 0) {
        return `We're still confirming your payment. Rechecking in ${_pendingCountdown}s.`;
      }
  ...
  return (props: any) => {
      ...
      fetchGateway("retry-payment", {
        ...
        .then((res) => {
          return res.json().catch(() => ({})).then((payload) => {
            if (!res.ok) {
              throw new Error(String(payload?.error || "Retry request failed"));
            }
            return payload;
          });
        })
        .then((payload) => {
          if (payload?.gateway === "razorpay" && payload?.razorpay?.order_id) {
            return ensureRazorpayScript().then(() => {
              ...
              const rzp = new (window as any).Razorpay({
                ...
                modal: {
                  ondismiss: () => {
                    setRetryMessage("Checkout was closed before payment completed. You can try again.");
                    setIsRetrying(false);
                  },
                },
              });
              rzp.on("payment.failed", () => {
                setRetryMessage("The payment attempt failed. You can try again.");
                setIsRetrying(false);
              });
              rzp.open();
            });
          ...
          throw new Error("Invalid Razorpay payment retry response");
        })
        .catch(() => {
          if (isFramerRuntimeHost()) {
            console.warn("[RetryPayment] Could not start payment checkout");
          }
          setRetryMessage("We couldn't start checkout. Please try again in a moment.");
        })
        .finally(() => {
          setIsRetrying(false);
        });
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "10px" }}>
        ...
        {retryMessage ? (
          <div role="status" style={{ color: "#b45309", fontSize: "14px", textAlign: "center" }}>
            {retryMessage}
          </div>
        ) : null}
      </div>
...
export function withHideWhenNoCoupon(Component): ComponentType {
  return (props: any) => {
    const [data, state] = useBooking();
    const hasCoupon = Boolean(data && appliedCouponCode(data));
