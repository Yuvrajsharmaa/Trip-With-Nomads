...
function buildLocalPricingBreakdown(store: CheckoutStore): PricingBreakdown {
  ...
  const couponWins = couponDiscount > 0 && (
    couponSource === "coupon" ||
    (couponSource !== "early_bird" && couponDiscount > earlyBirdDiscount)
  );
  ...
  return {
    ...
    early_bird_discount_amount: appliedEarlyBirdDiscount,
    coupon_discount_amount: appliedCouponDiscount,
    applied_discount_source: appliedCouponDiscount > 0
      ? "coupon"
      : appliedEarlyBirdDiscount > 0
        ? "early_bird"
        : "none",
    ...
    line_items: [
      ...
      ...(appliedEarlyBirdDiscount > 0
        ? [{
          ...
          amount: -appliedEarlyBirdDiscount,
      ...
      ...(appliedCouponDiscount > 0
        ? [{
          ...
          amount: -appliedCouponDiscount,
          type: "discount" as const,
        }]
        : []),
      { label: "GST (5%)", amount: tax, type: "tax" as const },
...
function buildPricingBreakdownFromQuote(
  ...
  const earlyBirdCandidate = toNumber(data.early_bird_discount_amount);
  const source = String(
  ...
  const couponCandidate = explicitCouponDiscount > 0
    ? explicitCouponDiscount
    : source === "coupon"
      ? rawDiscountTotal
      : 0;
  const couponWins = source === "coupon" ||
    (source !== "early_bird" && couponCandidate > earlyBirdCandidate);
  const appliedSource = couponWins
    ? "coupon"
    : earlyBirdCandidate > 0 || source === "early_bird"
      ? "early_bird"
      : "none";
  const couponDiscount = appliedSource === "coupon" ? couponCandidate : 0;
  const earlyBirdDiscount = appliedSource === "early_bird"
    ? earlyBirdCandidate > 0 ? earlyBirdCandidate : rawDiscountTotal
    : 0;
  const discountTotal = appliedSource === "coupon"
    ? couponDiscount
  ...
  return {
    ...
    applied_discount_code: appliedSource === "coupon"
      ? data.applied_discount_code || data.code || null
      : null,
    discount_amount_total: discountTotal,
    taxable_amount: taxableAmount,
...
function getAppliedCouponCode(store: CheckoutStore): string {
  ...
  if (String(totals.appliedDiscountSource || "").toLowerCase() !== "coupon") {
    return "";
  }
...
function getBookingAbandonLeadIdentity(tripId: string, email: string): string {
  const path = typeof window !== "undefined" ? window.location.pathname : "";
  return [tripId, email, path].join("|");
}

function getBookingAbandonLeadId(identity: string): string {
  const storageKey = BOOKING_ABANDON_LEAD_PREFIX + ":id:" + identity;
  try {
  ...
  try {
    ...
    // Storage is an optimization for deduplication, not a requirement.
  }
  return generated;
}

function hasBookingAbandonLeadBeenSent(identity: string): boolean {
  try {
    return sessionStorage.getItem(BOOKING_ABANDON_LEAD_PREFIX + ":sent:" + identity) === "1";
  } catch {
    return false;
  }
}

function markBookingAbandonLeadSent(identity: string) {
  try {
    sessionStorage.setItem(BOOKING_ABANDON_LEAD_PREFIX + ":sent:" + identity, "1");
  } catch {
    // The request has still succeeded even if storage is unavailable.
  }
}

function postBookingAbandonLead(store: CheckoutStore | null): Promise<boolean> {
  ...
  if (!tripId || !EMAIL_REGEX.test(email)) return Promise.resolve(false);

  const identity = getBookingAbandonLeadIdentity(tripId, email);
  if (hasBookingAbandonLeadBeenSent(identity) || bookingAbandonLeadInFlight.has(identity)) {
  ...
  return fetchGateway("record-lead", {
    ...
    keepalive: true,
    body: JSON.stringify({
      lead_id: getBookingAbandonLeadId(identity),
      email,
      name: name || null,
      phone: phone || null,
    ...
    .then(({ response, payload }) => {
      ...
      markBookingAbandonLeadSent(identity);
      return true;
    })
    .catch(() => false)
    .finally(() => {
        ...
        .then(({ res, data }) => {
          ...
          setStore({
            ...
            couponMessage: data.message ||
              (nextBreakdown.applied_discount_source === "coupon"
                ? `Coupon ${appliedCode || data.code || code} applied`
                : "Early-bird discount is better than this coupon"),
          });
...
export function withCouponMessage(Component): ComponentType {
  return (props: any) => {
    ...
    const appliedCode = getAppliedCouponCode(store);
    if (appliedCode) {
      text = `Coupon ${appliedCode} applied`;
    } else if (
      store.appliedCoupon?.valid &&
      String(totals.appliedDiscountSource || "").toLowerCase() === "early_bird"
    ) {
      text = "Early-bird discount is better than this coupon";
    }
    return <Component {...props} text={text} />;
  };
}
...
export function withCheckoutCouponCode(Component): ComponentType {
  return (props: any) => {
    ...
    const totals = computeTotals(store);
    const source = String(totals.appliedDiscountSource || "").toLowerCase();
    const couponAmount = source === "coupon" ? totals.couponDiscount : 0;
...
export function withCheckoutTaxLabel(Component): ComponentType {
  return withTextFromState(() => "GST (5%)")(Component);
}
...
export function withCheckoutGrandTotal(Component): ComponentType {
  return withTextFromState((store) => fmtINR(computeTotals(store).total))(
    Component,
  );
}
...
export function withCheckoutDueLabel(Component): ComponentType {
  return withTextFromState((store) =>
    computeTotals(store).paymentMode === "partial_25"
      ? "Remaining due"
      : "Balance due"
  )(Component);
}
...
export function withCheckoutHideWhenNoCoupon(Component): ComponentType {
  return (props: any) => {
    ...
    const totals = computeTotals(store);
    const source = String(totals.appliedDiscountSource || "").toLowerCase();
    const hasCouponAmount = source === "coupon" &&
      totals.couponDiscount > 0 &&
      Boolean(totals.appliedDiscountCode);
    ...
    const submit = () => {
      ...
      if (isFramerRuntimeHost()) {
        console.log("[Checkout] Pay initiated", {
          ...
          coupon: totals.appliedDiscountCode || null,
          total: totals.total,
      ...
      const payload = {
        ...
        coupon_code: totals.appliedDiscountCode || null,
        pricing_snapshot: {
      ...
      const executeCreateBooking = (body: string, isJson: boolean = true) => {
        ...
        return fetchCall
          ...
          .catch((err) => {
            if (isFramerRuntimeHost()) console.error("[Checkout] payment launch error", err);
            setStore({
              submitting: false,
              checkoutMessage: "Could not start payment",
            });
