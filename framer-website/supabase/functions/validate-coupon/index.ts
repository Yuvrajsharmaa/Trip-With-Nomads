...
Deno.serve(async (req) => {
    ...
    try {
        ...
        const departureDate = String(body?.departure_date || body?.date || "").trim()
        const fallbackTransport = String(body?.transport || body?.vehicle || "").trim()
        ...
        const couponWins = quote.applied_discount_source === "coupon"
        const message = couponWins
            ? `Coupon ${couponResult.code || couponCode} applied`
            : quote.applied_discount_source === "early_bird"
