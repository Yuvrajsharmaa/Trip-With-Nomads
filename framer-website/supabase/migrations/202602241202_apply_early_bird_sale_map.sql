-- Apply Early Bird Summer Sale discounts to mapped trip_pricing variants.
-- Requires columns added by 20260224_early_bird_pricing.sql.
...
target_slugs AS (
    SELECT DISTINCT slug FROM sale_map
),
updated AS (
...
FROM public.trips tr
WHERE tp.trip_id = tr.id
  AND tr.slug IN (SELECT slug FROM target_slugs)
  AND tp.id NOT IN (SELECT id FROM updated);
